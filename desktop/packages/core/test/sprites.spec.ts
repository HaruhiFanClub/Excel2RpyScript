...
import { parseSprites, serializeSprites, slotOf } from '../src/sprites'

describe('立绘三列：解析/序列化', () => {
  it('真实数据形式（<角色>_left/_mid/_right）往返一致', () => {
    for (const s of [
      'kyon 0012 kyon_left',
      ...
      expect(serializeSprites(parseSprites(s))).toBe(s)
    }
  })
  ...
  it('内置位置会规整为每角色位置 token', () => {
    // 'right' 内置 → 序列化为 <角色>_right（编辑器规范化）
    const s = parseSprites('sanmisen 0001 right')
    expect(s.right).toBe('sanmisen 0001')
    expect(serializeSprites(s)).toBe('sanmisen 0001 sanmisen_right')
  })
  ...
  it('编辑模型：填左列 → 生成带位置的段', () => {
    ...
    expect(out).toBe('kyon 0030 kyon_left')
  })
