...
- **转换**：`@e2r/core` TS 重写，与旧工具**逐字符对齐**（黄金回归 + 对齐陷阱单测）。转换页生成/导出 .rpy。
- **表格**：AG Grid 多 sheet 浏览/编辑，保存回写 .xlsx（保留样式/合并）；关联工程后单元格直接预览**立绘/背景缩略图、音乐/音效播放**；**立绘拆左/中/右三列**（只填角色+编号，位置按角色自动匹配）。
- **语音合成**：内置 GPT-SoVITS 引擎托管（一键启动）+ 自定义端点；逐句 角色/语气/状态，批量/单句合成、试听、**未重新生成**提示；合成流程与旧版一致并修正 sheet 序号 bug。
- **检查 / 对比**：error/warn/info 三级检查（含音乐/立绘/背景太久未换）；新旧表 Diff 差异报告。
- **工程**：关联 Ren'Py 工程，一键把 .rpy 写入 game/、启用语音，打通代码→资源路径。
- **UI**：玻璃拟态、浅/深双主题、共享 WorkspaceBar。
- **分发**：见 `docs/build-distribution.md`，一条命令产出全离线内置 TTS 的安装包。

测试 **71 项通过**（`pnpm -r test`）。参考工程仅作风格参考，各页原创设计。

## 后续可深化
- 每角色立绘位置配置 UI + 读取 renpy transform 校正；TTS 配置可视化编辑 + 角色多别名绑定；表格内语音指令下拉。
- `.e2rproj` 工程文件（持久化设置/每行状态/快照）。
- 关联工程后的 renpy 实跑校验；非法 Excel 导入即时提示。
- 分发签名/公证与 CI 矩阵、体积优化。
