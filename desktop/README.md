...
- 黄金回归 + reader 对齐 + 落盘字节比对 + 对齐陷阱单测全绿。
- Electron 壳（玻璃拟态 UI，浅/深双主题）：转换工作台（预览/导出/模式/告警）。

**M1 进行中**
- ✅ 检查器（core）：error/warn/info 三级，音乐/立绘/背景"太久未换"告警。
- ✅ 表格编辑（AG Grid 玻璃主题）：多 sheet 浏览/编辑，保存回写 .xlsx（保留样式/合并）。
- ⏳ 待办：立绘左/中/右三列、检查结果 UI 面板、`.e2rproj` 工程模型、新旧表 diff。

测试合计 **51 项通过**（`pnpm -r test`）。UI 风格见上层记忆/约定：参考工程仅作风格参考，各页原创设计。
