...
- **工程联动**：导入工作簿后使用 workspace 副本作为活动表；顶部栏关联 Ren'Py 工程后，转换页可按脚本应用到 game/，TTS 页可将已应用语音同步到 game/audio/。
- **导入校验**：非法模板即时红条提示。
...
- 每角色立绘位置的可视化编辑（留空用 `left/right/mid`；凉宫内置角色已配置 `<角色>_left/_mid/_right` 前缀 + 关联工程后校验）。
...
- **M1** 可编辑多 sheet 表单（AG Grid）+ ExcelJS 就地写回保样式/合并 + workspace 副本 + 校对器 + diff。
- **M2** 立绘左/中/右三列（有序 sprite 模型，无损往返）+ renpy 工程关联（读 transform）+ 资源缩略图。
- **M3** 内置 GPT-SoVITS TTS（冻结子进程）+ 批量/单句合成 + 试听 + 语气映射 + 「未重新生成」追踪。
- **M4** 全离线分发（PyInstaller 冻结 + electron-builder）+ 工程联动收尾。
